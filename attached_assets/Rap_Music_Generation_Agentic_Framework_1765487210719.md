...
### **5.3 Node Logic: The Sonauto Bridge (Generation & Inpainting)**
...
def sonauto\_node(state: RapGenerationState):  
    ...
    \#... (Polling logic implemented here or in separate node)...  
...
### **5.5 Graph Connectivity and Routing**
...
\# Add Nodes  
workflow.add\_node("LyricalArchitect", lyric\_node)  
workflow.add\_node("SonautoGen", sonauto\_node)  
workflow.add\_node("FlowSupervisor", analysis\_node)  
workflow.add\_node("MixEngineer", mastering\_node)
...
def route\_after\_analysis(state):  
    if state\["inpaint\_segments"\]:  
        return "SonautoGen" \# Loop back for fixing  
    elif state\["is\_complete"\]:  
